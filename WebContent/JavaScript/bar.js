...
function validateAndRedirect() {
    ...
    if (!token || !isValidJWT(token)) {
        sessionStorage.removeItem('jwt'); // Remove invalid or missing token
...
document.addEventListener('visibilitychange', function () {
    if (document.visibilityState === 'hidden') {
        // Start a timer to log out after a short delay
        logoutTimeout = setTimeout(() => {
            console.log('Tab switched or minimized. Logging out...');
            logoutUser();
        }, 1000); // 2-second delay
    }
});
...
window.addEventListener('popstate', function () {
    // Cancel the logout timeout if the user navigates within the app
