...
document.addEventListener('DOMContentLoaded', function () {
    ...
    async function saveChanges() {
        ...
        if (Object.keys(updatedProfile).length === 0) {
            showNotification("No changes detected for update.", "warning");
            return;
        }
        ...
        try {
            ...
            if (result.status==200) {
                showNotification("Employee profile updated successfully!"); // Changed alert message
                closeModal(); // Close modal after save
                fetchAccounts(); // Refresh employee list
            } else {
                showNotification(result.message || "Failed to update employee profile.", "warning"); // Changed alert message
            }
        } catch (error) {
            console.error(error);
            showNotification("An error occurred while updating the employee profile.", "error"); // Changed alert message
        }
    }
    ...
    document.getElementById('searchForm').addEventListener('submit', async function (event) {
        ...
        try {
            ...
            showNotification("Failed to retrieve employees.", "error"); // Changed alert message
        }
    });
    ...
    async function initialize() {
        try {
            ...
            if (!branchId) {
                showNotification("Branch ID not found in token.", "warning");
                return;
            }
            ...
            showNotification("Failed to initialize employee list.", "error");
        }
    }
    initialize();
